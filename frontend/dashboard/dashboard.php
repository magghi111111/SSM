    ...
    <main class="main">
        ...
        <section class="kpi-section">
            ...
            <div class="kpi-card alert">
                <a href="index.php?page=avvisi" style="text-decoration: none; color: inherit;">
                    ...
                    <p class="kpi-value" id="kpi-avvisi">—</p>
                </a>
            </div>
        ...
        <?php if ((isset($_SESSION['permessi']['andamenti']) && $_SESSION['permessi']['andamenti'])
            ...
            <section class="content-grid">
                <!-- PREVISIONI -->
                <?php if (isset($_SESSION['permessi']['previsioni']) && $_SESSION['permessi']['previsioni']): ?>
                    <div class="card">
                        <h2>Previsioni Vendite Dei Prossimi 30 Giorni</h2>
                        
                        <div class="placeholder">
                            <iframe
                                src="frontend/previsioni/grafico.php"
                                scrolling="no">
                            </iframe>
                        </div>
                    </div>
...
<script>
    // Esegue il refresh degli avvisi in background, senza bloccare la pagina
    fetch('backend/controller/avvisiCntrl.php');

    // Poi carica il conteggio aggiornato
    fetch('backend/api/countAvvisi.php')
        .then(r => r.json())
        .then(data => {
            document.getElementById('kpi-avvisi').textContent = data.count;
        });
</script>
